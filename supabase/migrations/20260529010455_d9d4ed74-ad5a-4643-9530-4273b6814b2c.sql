...
BEGIN
  ...
  SELECT alert_type_value::public.alert_type, message_value, item_name_value, urgent_value, false
  WHERE NOT EXISTS (
...
GRANT EXECUTE ON FUNCTION public.create_inventory_alert(text, text, text, boolean) TO service_role;

-- Update dispatch_product to cast to enum explicitly
CREATE OR REPLACE FUNCTION public.dispatch_product(product_id_value uuid, quantity_value integer, batch_id_value uuid DEFAULT NULL::uuid, dispatch_type_value text DEFAULT 'sale'::text, destination_value text DEFAULT NULL::text, reference_number_value text DEFAULT NULL::text, unit_price_value numeric DEFAULT NULL::numeric, dispatched_date_value date DEFAULT NULL::date, notes_value text DEFAULT NULL::text)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF product_row.quantity - quantity_value <= product_row.min_stock THEN
    PERFORM public.create_inventory_alert(
      (CASE WHEN product_row.quantity - quantity_value <= 0 THEN 'critical' ELSE 'low-stock' END)::public.alert_type,
...
-- Update produce_batch to cast to enum explicitly
CREATE OR REPLACE FUNCTION public.produce_batch(product_id_value uuid, quantity_value integer, expiration_date_value date, batch_code_value text DEFAULT NULL::text, production_date_value date DEFAULT CURRENT_DATE)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  LOOP
    ...
    IF (ingredient_row.current_stock - ingredient_row.required_quantity) <= ingredient_row.min_stock THEN
      PERFORM public.create_inventory_alert('low-stock'::public.alert_type,
...
-- Update log_defect to cast to enum explicitly
CREATE OR REPLACE FUNCTION public.log_defect(batch_id_value uuid, quantity_value integer, reason_value text DEFAULT NULL::text)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
